module MAC_rx_ctl_top(
    ...
    output wire [7:0] mac_rx_fifo_din,//输入数据87ui
...
CRC_check CRC_check_inst(
	//--------------------------系统信号------------------------------------------
	.clk     (gmii_rx_clk),		//接受时钟信号  input wire 
	.sys_rst_n  (sys_rst_n),	//系统复位信号  input wire 
	//---------------------------------------------------------------------------

	//
	.data       (),	//输入待校验的8位数据   input wire [7:0] 
	.crc_en     (),		//crc使能，开始校验标志input wire 
	.crc_clr    (),		//crc数据复位信号input wire 
	
	.crc_data   (),		//CRC校验数据       output reg [31:0] 
	.crc_next   (),	//CRC下次校验完成数据    output wire [31:0] 
	.crc_err    ()    //CRC校验错误信号            output reg 
	//
);
